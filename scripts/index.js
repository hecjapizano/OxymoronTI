...
Data.prototype.step = function(){
	...
	const {z, x, y} = this.screen;
	this.screen.lastX = x;
	this.screen.lastY = y;
	this.setScreen();
	// noinspection JSCheckFunctionSignatures
	this["screen"].color = opts.dataColor.replace("light", 40 + (tick * this.glowSpeed) % 50).replace(.2 + (1 - mostDistant / z) * .6, "alp");
...
window.addEventListener( 'resize', function(c){
	...
	opts.vanishPoint.x = 2 / (w = c.width = window.innerWidth);
	opts.vanishPoint.y = 2 / (h = c.height = window.innerHeight);
	ctx.fillRect( 0, 0, w, h );
});
